...
function Particles.draw()
   ...
   for _, particle in pairs(Particles.active) do
      if love.timer.getTime() > particle.creation_time + particle.duration then
         ...
         local x = interpolate(pos1.x, pos2.x, particle.velocity_decay, t0, d)
         local y = interpolate(pos1.y, pos2.y, particle.velocity_decay, t0, d)
         local size = interpolate(particle.size, 0, particle.size_decay, t0, d)
